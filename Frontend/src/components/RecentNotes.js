...
function RecentNotes() {
  ...
  // To display the last three added notes
  const recentNotes = notes.slice(-3).reverse() // reverse to display the last added note first
  return (
    <div>
      <button
        ref={ref}
      ...
      <div className="recent-note-container">
        {recentNotes.length === 0 ? (
          ...
          recentNotes.map((note) => {
            return <RecentNoteItem note={note} update={updateNote} />;
          })
        )}
        </div>
        </div>
